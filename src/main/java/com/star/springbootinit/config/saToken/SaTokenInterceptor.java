        ...
        registry.addInterceptor(new SaInterceptor(handler -> {
                    ...
                    SaRouter
                            ...
                            .check(r -> {
                                        ...
                                        StpUtil.checkLogin();
                                        // 查看token是否要过期
                                        long tokenSessionTimeout = StpUtil.getTokenSessionTimeout();// 获取当前 Token-Session 剩余有效时间 (单位: 秒)
                                        // 如果小于一天 那么就进行续期
                    ...
                    // 根据路由划分模块，不同模块不同鉴权
                    SaRouter.notMatch("/**/login/**").match("/user/**", r -> StpUtil.checkPermission("user"));
